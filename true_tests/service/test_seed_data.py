...
class TestEmptyDatabaseSeeding:
    ...
    def test_seed_if_empty_adds_expected_storylets(self):
        """Test that seed_if_empty() adds the expected number of storylets to empty database."""
        ...
        # Verify expected number of storylets added
        final_count = self.db.query(Storylet).count()
        assert final_count == 7, f"Expected 7 storylets, but got {final_count}"
    
    def test_seed_if_empty_does_not_seed_non_empty_database(self):
        """Test that seed_if_empty() does not add storylets if database already has data."""
        # Add one storylet manually
        ...
        # Verify no additional storylets were added
    ...
    def test_seeded_storylets_have_correct_structure(self):
        """Test that seeded storylets have proper structure and required fields."""
        # Run seeding
        seed_if_empty(self.db)
        
        # Get all seeded storylets and verify basic structure
        storylets = self.db.query(Storylet).all()
        
        assert len(storylets) == 7, "Should have 7 seeded storylets"
        
        # Test that all storylets have basic required data
        for storylet in storylets:
            ...
            # Non-empty checks by converting to expected types
    ...
    def test_seeded_storylets_specific_content(self):
        """Test that specific expected storylets are present with correct content."""
        seed_if_empty(self.db)
        
        storylets = self.db.query(Storylet).all()
        storylet_titles = [str(s.title) for s in storylets]
        
        # Check for specific expected storylets
        expected_titles = [
            "A Dark Cave",
            "Mysterious Stranger", 
            "Abandoned Hut",
    ...
    def test_seeded_storylets_requirements_and_choices(self):
        """Test that seeded storylets have proper requirements and choice structures."""
        seed_if_empty(self.db)
        
        # Test specific storylet requirements
        dark_cave = self.db.query(Storylet).filter_by(title="A Dark Cave").first()
        assert dark_cave is not None
        
        # Test that requirements and choices are properly structured
        assert isinstance(dark_cave.requires, dict)
        assert isinstance(dark_cave.choices, list)
        assert len(dark_cave.choices) >= 1
        
        # Test complex requirements storylet
    ...
    def test_seed_multiple_calls_idempotent(self):
        """Test that calling seed_if_empty multiple times doesn't add duplicate storylets."""
        # First seeding
        seed_if_empty(self.db)
        ...
        # Third seeding (should still be no-op)
        seed_if_empty(self.db)
        third_count = self.db.query(Storylet).count()
        assert third_count == 7, "seed_if_empty should remain idempotent"
    
    def test_seeded_storylets_database_persistence(self):
        """Test that seeded storylets persist properly in database."""
        # Seed the database
        seed_if_empty(self.db)
        ...
        try:
            ...
            # Verify specific storylet persists with all data
    ...
    def test_seed_with_transaction_rollback(self):
        """Test seeding behavior with transaction rollbacks."""
        # Start a transaction
        self.db.begin()
        ...
        try:
            ...
            # Verify storylets are there within transaction
            ...
            # Verify storylets are gone after rollback
    ...
    def test_storylet_choice_format_consistency(self):
        """Test that all seeded storylets have consistent choice formats."""
        seed_if_empty(self.db)
        ...
        for storylet in storylets:
            ...
            for choice in choices:
                ...
                # Each choice should have either 'label' or 'text' (but not both as primary)
                ...
                # Each choice should have either 'set' or 'set_vars' (but not both as primary)
