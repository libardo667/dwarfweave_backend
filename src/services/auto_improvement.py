...
def auto_improve_storylets(
    ...
    try:
        ...
        smoothing_total = sum([
            v for k, v in results['smoothing_results'].items() 
            if isinstance(v, int)
        ])
        deepening_total = sum([
            v for k, v in results['deepening_results'].items() 
            if isinstance(v, int)
        ])
        
        results['total_improvements'] = smoothing_total + deepening_total
        
        print(f"🎉 Auto-improvement complete! Total improvements: {results['total_improvements']}")
        
        # Log the improvement for admin visibility
        if db:
            logging.info(f"Auto-improvement completed for {trigger}: {results['total_improvements']} improvements")
